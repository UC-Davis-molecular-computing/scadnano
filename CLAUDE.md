...
- **`CHANGELOG.md` is auto-generated** by the `changelog` GitHub Actions workflow from the GitHub releases.
  Never edit it by hand (the next run overwrites the whole file); edit the release notes instead. Do not read
  it either — it is very large and adds nothing over the release notes.