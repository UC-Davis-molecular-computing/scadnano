...
Map<int, Point<double>> helices_assign_svg(Design design, bool invert_y, BuiltMap<int, Helix> helices,
...
double distance_to_rectangle(Point<double> point, Point<double> upper_left_corner, double width,
...
Point<double> xy_distances_to_rectangle(Point<double> point, Point<double> upper_left_corner, double width,
    double height, double angle) {
...
Point<double> closest_point_in_rectangle(Point<double> point, Point<double> upper_left_corner, double width,
    double height, double angle) {
  var distances = xy_distances_to_rectangle(point, upper_left_corner, width, height, angle);
  Point<double> unrotated_point = rotate(point, -angle, origin: upper_left_corner);
...
/// Return (closest) helix, offset and direction where click event occurred.
Address find_closest_address_with_infinite_helix_boundaries(MouseEvent event,
    Helix helix,
    BuiltMap<String, HelixGroup> groups,
    Geometry geometry,
...
Point<double> transform_mouse_coord_to_svg_current_panzoom_correct_firefox(MouseEvent event,
    bool is_main_view, SvgSvgElement view_svg) {
  Point<double> point;
  if (!browser.isFirefox) {
...
Position3D svg_side_view_to_position3d(Point<double> svg_pos, bool invert_y, Geometry geometry) =>
    Position3D(
...
Point<double> compute_extension_attached_end_svg(Extension ext, Domain adj_dom, Helix adj_helix,
    num adj_helix_svg_y, Geometry geometry) {
...
Point<double> compute_extension_free_end_svg(Point<double> attached_end_svg, Extension ext,
    Domain adjacent_domain, Geometry geometry) {
