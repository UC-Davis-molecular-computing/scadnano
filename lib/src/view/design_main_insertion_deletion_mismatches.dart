...
UiFactory<DesignMainInsertionDeletionMismatchesProps> DesignMainInsertionDeletionMismatches =
    _$DesignMainInsertionDeletionMismatches;

mixin DesignMainInsertionDeletionMismatchesProps on UiProps {
  Design design;
...
class DesignMainInsertionDeletionMismatchesComponent
    extends UiComponent2<DesignMainInsertionDeletionMismatchesProps> with PureComponent {
  @override
  render() {
  ...
  List<ReactElement> _create_mismatch_components() {
    ...
    for (Strand strand in props.design.strands) {
      for (Domain domain in strand.domains) {
        BuiltList<Mismatch> mismatches = props.design.insertion_deletion_mismatches_on_domain(domain);
        ...
        for (Mismatch mismatch in mismatches) {
          ...
          if (!props.only_display_selected_helices || props.side_selected_helix_idxs.contains(helix.idx)) {
            var base_svg_pos = helix.svg_base_pos(
                mismatch.offset, domain.forward, props.helix_idx_to_svg_position_y_map[helix.idx]);
