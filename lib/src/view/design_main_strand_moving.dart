...
//        (ConnectedDesignMainStrandPaths()
      ...
      (EndMoving()
        ...
        ..forward = first_ss.forward
        ..helix = props.helices[first_ss.helix]
        ..is_5p = true
        ..allowable = props.allowable
        ..current_offset = end_5p.offset_inclusive + props.delta
        ..key = 'end-5p')(),
      (EndMoving()
        ..dna_end = end_3p
        ..original_offset = end_3p.offset_inclusive
        ..color = props.strand.color
        ..forward = last_ss.forward
        ..helix = props.helices[last_ss.helix]
        ..is_5p = false
        ..allowable = props.allowable
        ..current_offset = end_3p.offset_inclusive + props.delta
        ..key = 'end-3p')(),
    ]);
  }
  ...
  ReactElement _draw_strand_lines_single_path() {
    ...
    int delta = props.delta;

    var helix = props.helices[substrand.helix];
    var start_svg = helix.svg_base_pos(substrand.offset_5p + delta, substrand.forward);
    ...
    for (int i = 0; i < bound_substrands.length; i++) {
      // substrand line
      var end_svg = helix.svg_base_pos(substrand.offset_3p + delta, substrand.forward);
      ...
      if (i < bound_substrands.length - 1) {
        ...
        start_svg = helix.svg_base_pos(substrand.offset_5p + delta, substrand.forward);
        var control = control_point_for_crossover_bezier_curve(old_substrand, substrand, delta: delta);
