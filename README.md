...
* **(p)encil:**
  ...
  In any mode, if two bound substrands on adjacent helices have their 5'/3' ends at the same horizontal offset, by placing the cursor over where a crossover between them would appear, a potential crossover appears, which can be left-clicked to add a crossover.
